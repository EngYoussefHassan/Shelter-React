      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden md:flex space-x-6">
          ...
          <div className="relative">
            <button onClick={toggleDropdown} className="hover:text-blue-700 focus:outline-none">How To</button>
            {isDropdownOpen && (
        ...
        <div className="flex items-center space-x-4">
          {user ? (
            <button onClick={handleLogout} className="hidden md:block rounded-[5px] bg-blue-950 text-white text-center px-4 py-2 hover:bg-blue-800">Log Out</button>
          ) : (
            <>
              <button onClick={() => openAuthModal("signIn")} className="hidden md:block rounded-[5px] bg-blue-950 text-white text-center px-4 py-2 hover:bg-blue-800">Sign Up</button>
              <button onClick={() => openAuthModal("logIn")} className="hidden md:block rounded-[5px] bg-blue-950 text-white text-center px-4 py-2 hover:bg-blue-800">Log In</button>
            </>
          )}
        </div>
      ...
      {authModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96 relative">
            <button onClick={closeAuthModal} className="absolute top-2 right-2 text-gray-500 hover:text-gray-700">&times;</button>
            <h2 className="text-xl font-bold text-gray-800 mb-4">
              {authModal === "signIn" ? "Sign Up" : "Log In"}
            </h2>
            <Auth authType={authModal} /> {/* Pass the auth type to the Auth component */}
          </div>
        </div>
      )}
    </nav>
